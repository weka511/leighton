...
from kepler.kepler     import get_mean_anomaly,get_eccentric_anomaly,get_true_anomaly,get_distance_from_focus,true_longitude_from_true_anomaly
from kepler.solar      import Solar
from math              import radians
from matplotlib.pyplot import plot, show
from physics           import  CO2, Radiation
from planet            import Planet
from utilities         import InternalTemperatureLog, slip_zip, TemperatureRecord
...
class Surface(Layer):
    ...
    def update_temperature_adjust_for_co2(self,total_inflow_before_latent_heat,dT):
        ...
        '''
        if self.temperature>CO2.condensation_temperature:
    ...
    def bolzmann(self,t):
        ...
        return self.emissivity()*Radiation.bolzmann(t)

    def absorption(self):
        '''Calculate absorbed energy, assuming frozen CO2 affects albedo'''
        return 1-CO2.albedo  if self.total_co2>0  else self.planet.F
...
class ThermalModel:
    ...
    def stable_temperature(solar,planet,proportion=0.25):
        ...
        return Radiation.reverse_bolzmann(beam_irradience)
    ...
    def __init__(self,latitude,spec,solar,planet,history,temperature,co2):
        ...
        self.record=None
        self.zipper_layers = list(slip_zip(self.layers))
    ...
    def runModel(self,start_day,number_of_days,number_of_steps_in_hour):
        ...
        for day,hour,_,step,_,_,_,_,_,true_longitude in steps(0,number_of_days,10,self.planet):
            self.record = TemperatureRecord(day,hour,hours_in_day,true_longitude)
    ...
    for day in range(start_day,start_day+number_of_days):
        for hour in range(24):
            for step in range(number_of_steps_in_hour):
                ...
                M                      = get_mean_anomaly(1,radians(time))
                E                      = get_eccentric_anomaly(M,planet.e)
                nu                     = get_true_anomaly(E,planet.e)
                r                      = get_distance_from_focus(nu,planet.a,planet.e)
                true_longitude         = true_longitude_from_true_anomaly(nu,PERH=planet.longitude_of_perihelion)
                yield (day,hour,hour_with_step,step,time,M,E,nu,r,true_longitude)

if __name__=='__main__':
    mars    = Planet.create('Mars')
    solar   = Solar(mars)
    history = InternalTemperatureLog()
    ...
    plot(days,surface_temp,'r-',days,t1,'g-',days,t2,'b-',days,t3,'c-',days,t4,'m-',days,t5,'y-',days,t6,'b-',days,t7,'b-')
    show()
